...
const COURIERS = [
  ...
  { id: "rpx", name: "RPX" },
  { id: "lex", name: "Lazada Express" },
  { id: "indah_cargo", name: "Indah Cargo" },
  { id: "dakota", name: "Dakota Cargo" },
  { id: "kurir_tokopedia", name: "Kurir Rekomendasi" },
];
...
const guessCourierPriority = (courierId, resi) => {
  ...
  switch (courierId) {
    ...
    case "jnt":
      if (
        ["JP", "JX", "JZ", "JD", "JT", "JB"].some((prefix) =>
          upperResi.startsWith(prefix),
        )
      )
        return 3;
    ...
    case "jne":
      if (
        ["TJE", "JNA", "CGK", "SOC", "BDO", "SUB"].some((prefix) =>
          upperResi.startsWith(prefix),
        )
      )
        return 3;
    ...
    case "ninja":
      if (
        ["NLID", "NVID", "NINJA", "ZILGO"].some((prefix) =>
          upperResi.startsWith(prefix),
        )
      )
        return 3;
    ...
    case "tiki":
      ...
      break;
    case "ide":
      if (upperResi.startsWith("IDS")) return 3;
      break;
    case "lex":
      if (upperResi.startsWith("LXAD") || upperResi.startsWith("LEX")) return 3;
      break;
    case "kurir_tokopedia":
      if (upperResi.startsWith("TKP")) return 3;
      break;
    default:
      return 0;
  }
...
export default function Home() {
  ...
  const handleTrack = async (e) => {
    ...
    let couriersToTry = [...COURIERS].sort((a, b) => {
      return (
        guessCourierPriority(b.id, resi) - guessCourierPriority(a.id, resi)
      );
    });
    ...
    for (let i = 0; i < couriersToTry.length; i++) {
      ...
      try {
        ...
        const hasValidHistory =
          ...
          (history[0].desc || history[0].date);

        if (response.data?.status === 200 && hasValidHistory) {
          if (
            courier.id === "jne" &&
            history[0].desc &&
            history[0].desc.includes("UNTUK MENDAPATKAN HISTORY LENGKAP")
          ) {
            setJnePhoneReq(true);
          } else {
            setJnePhoneReq(false);
    ...
    if (!found) {
      setErrorMsg(
        "Nomor resi tidak ditemukan di semua ekspedisi. Pastikan nomor resi benar.",
      );
    }
    ...
    try {
      ...
      if (response.data?.status === 200 && hasValidHistory) {
        if (
          ...
          setErrorMsg(
            "Gagal mendapatkan history. 5 digit terakhir nomor HP mungkin salah.",
          );
        } else {
          setJnePhoneReq(false);
          setTrackingData(responseData);
    ...
    <div className="flex-grow flex flex-col items-center justify-start w-full transition-all duration-500 animate-slide-up">
      ...
      <div className="text-center mt-10 mb-12">
        ...
        <p className="text-slate-500 font-medium max-w-lg mx-auto">
          Cukup masukkan SATU nomor resi. Sistem kami akan mendeteksi dari
          sekian banyak ekspedisi secara otomatis.
        </p>
      </div>
      ...
      <form onSubmit={handleTrack} className="w-full max-w-2xl relative z-20">
        ...
        {/* Loading Indicator */}
        <div
          className={`mt-6 text-center transition-all duration-300 ${loading ? "opacity-100" : "opacity-0 h-0 hidden"}`}
        >
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-indigo-600 font-semibold text-sm">
              Sedang mencoba: {checkingCourier}...
            </p>
          </div>
        </div>
          ...
          <div className="glass-card p-6 md:p-8 mb-8 relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row justify-between gap-6">
              <div className="min-w-0 flex-1">
                <h2 className="text-xl font-bold text-slate-800 mb-1 truncate">
                  {trackingData.summary?.awb}
                </h2>
                <p className="text-slate-500 text-sm font-medium mb-4">
                  Dikirim via {trackingData.summary?.courier}
                </p>

                <div className="flex flex-col gap-2 mt-4 text-sm w-full">
                  <div className="flex items-start gap-2 w-full">
                    <span className="text-slate-400 font-medium min-w-[70px] shrink-0">
                      Pengirim:
                    </span>
                    <span className="text-slate-700 font-semibold break-all min-w-0 flex-1">
                  ...
                  <div className="flex items-start gap-2 w-full">
                    ...
                    <span className="text-slate-700 font-semibold break-all min-w-0 flex-1">
                      {trackingData.detail?.receiver || "-"} <br />{" "}
              ...
              <div className="flex flex-col items-start md:items-end md:justify-center border-t md:border-t-0 md:border-l border-slate-200 pt-4 md:pt-0 md:pl-6 text-left md:text-right shrink-0">
                <div className="text-slate-400 font-medium text-xs uppercase tracking-wider mb-1">
                  Status Terkini
                </div>
                <div
                  className={`px-4 py-2 rounded-lg font-bold text-sm mb-2 shadow-sm
                      ${
                        trackingData.summary?.status === "DELIVERED"
              ...
              <div className="flex items-start gap-4 flex-col md:flex-row">
                ...
                <div className="flex-grow w-full">
                  <h3 className="text-lg font-bold text-slate-800 mb-1">
                    Dibutuhkan Nomor Handphone (JNE)
                  </h3>
                  <p className="text-sm text-slate-600 mb-4 leading-relaxed">
                    Pihak ekspedisi menyembunyikan riwayat lengkap untuk
                    keamanan. Masukkan <strong>5 digit terakhir</strong> nomor
                    HP penerima.
                  </p>

                  <form
                    onSubmit={handleTrackJneWithPhone}
                  ...
                  >
                    <div className="relative flex-grow">
                      ...
                      <input
                        ...
                        value={jnePhone}
                        onChange={(e) =>
                          setJnePhone(e.target.value.replace(/\D/g, ""))
                        }
                        disabled={loading}
                      />
                    </div>
